...
// --- CABINET SVG --------------------------------------------------------------
// handleKey: black_bar | brass_cup | brass_bar | integrated | chrome_bar
function CabSVG({tid, cs, doorColor, doorKey="supawood_white", sel, handleKey="black_bar"}){
  const cab    = cabDef(tid);
  const w      = cab.wC * cs;
  const h      = cab.hC * cs;
  const dc     = doorColor || "#E8E0D0";
  const isWood = PM.doors[handleKey] ? false : ["veneer_oak","veneer_walnut"].includes(handleKey);
  const stroke = sel ? B.orange : "#7A6A58";
  const sw     = sel ? 2 : 1;
  ...
  const pad = Math.max(2, Math.round(cs * 0.18));

  // -- Handle renderer ----------------------------------------------------------
  // hk = handleKey from PM.handles
  function Handle({x, y, dw, dh, isDrawerFront=false}){
    const hk = handleKey;
    const cx = x + dw/2;
    // Vertical position: 72% down for doors, centred for drawer fronts
    const hy = isDrawerFront ? y + dh/2 : y + dh*0.70;

    if(hk==="integrated"){
      // J-pull: recessed channel at very top of door
      const jy = y + dh*0.08;
      return <rect x={x+dw*0.1} y={jy} width={dw*0.8} height={Math.max(2,dh*0.07)}
        fill="rgba(0,0,0,0.18)" rx="1"/>;
    }
    if(hk==="brass_cup"){
      // Cup handle: small circular cup
      const r = Math.max(3, dw*0.09);
      return <g>
        <circle cx={cx} cy={hy} r={r} fill="none" stroke="#C8A850" strokeWidth={Math.max(1.2,cs*0.09)}/>
        <circle cx={cx} cy={hy} r={r*0.45} fill="#C8A850" opacity="0.5"/>
      </g>;
    }
    if(hk==="brass_bar" || hk==="chrome_bar"){
      const barColor = hk==="brass_bar" ? "#C8A850" : "#B8B8C0";
      const barW = isDrawerFront ? dw*0.55 : Math.max(4, dw*0.12);
      const barH = isDrawerFront ? Math.max(2,dh*0.14) : Math.max(6, dh*0.32);
      return isDrawerFront
        ? <rect x={cx-barW/2} y={hy-barH/2} width={barW} height={barH} fill={barColor} rx={barH/2} opacity="0.9"/>
        : <rect x={cx-barW/2} y={hy-barH/2} width={barW} height={barH} fill={barColor} rx={barW/2} opacity="0.9"/>;
    }
    // default: black_bar
    const barW = isDrawerFront ? dw*0.55 : Math.max(4, dw*0.12);
    const barH = isDrawerFront ? Math.max(2,dh*0.14) : Math.max(6, dh*0.32);
    return isDrawerFront
      ? <rect x={cx-barW/2} y={hy-barH/2} width={barW} height={barH} fill="#2A1810" rx={barH/2} opacity="0.85"/>
      : <rect x={cx-barW/2} y={hy-barH/2} width={barW} height={barH} fill="#2A1810" rx={barW/2} opacity="0.85"/>;
  }

  // -- Door panel with reveal and depth -----------------------------------------
  function Door({x, y, dw, dh, isDrawerFront=false}){
    const reveal = Math.max(1.5, cs*0.12);
    return(
      <g>
        {/* Carcass reveal shadow */}
        <rect x={x} y={y} width={dw} height={dh} fill="rgba(0,0,0,0.10)" rx="1"/>
        {/* Door face -- inset */}
        <rect x={x+reveal} y={y+reveal} width={dw-reveal*2} height={dh-reveal*2} fill={dc} rx="1"/>
        {/* Top-left highlight edge */}
        <line x1={x+reveal} y1={y+reveal} x2={x+dw-reveal} y2={y+reveal} stroke="rgba(255,255,255,0.25)" strokeWidth="0.8"/>
        <line x1={x+reveal} y1={y+reveal} x2={x+reveal} y2={y+dh-reveal} stroke="rgba(255,255,255,0.25)" strokeWidth="0.8"/>
        {/* Bottom-right shadow edge */}
        <line x1={x+dw-reveal} y1={y+reveal} x2={x+dw-reveal} y2={y+dh-reveal} stroke="rgba(0,0,0,0.15)" strokeWidth="0.8"/>
        <line x1={x+reveal} y1={y+dh-reveal} x2={x+dw-reveal} y2={y+dh-reveal} stroke="rgba(0,0,0,0.15)" strokeWidth="0.8"/>
        {/* Wood grain lines for veneer */}
        {["veneer_oak","veneer_walnut"].includes(doorKey) && [0.25,0.5,0.75].map((t,i)=>(
          <line key={i} x1={x+reveal+dw*t*0.7} y1={y+reveal+2} x2={x+reveal+dw*t*0.7+dw*0.04} y2={y+dh-reveal-2}
  ...
  let doors;
  if(cab.isDrawer){
    const n=3, gap=pad, totalGap=gap*(n+1), frontH=(bodyH-totalGap)/n;
    doors=<>{[0,1,2].map(i=>(
    ...
    const sectionH=bodyH/2 - pad*1.5;
    const halfW=(w-pad*3)/2;
    doors=<>
      <Door x={pad}          y={bodyTop+pad}   dw={halfW} dh={sectionH}/>
      <Door x={pad*2+halfW}  y={bodyTop+pad}   dw={halfW} dh={sectionH}/>
      <Door x={pad}          y={midY+pad/2}    dw={halfW} dh={sectionH}/>
      <Door x={pad*2+halfW}  y={midY+pad/2}    dw={halfW} dh={sectionH}/>
    </>;
  ...
  return(
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{display:"block",pointerEvents:"none"}}>
      {/* Carcass body with subtle gradient depth */}
      <defs>
        <linearGradient id={`cg${tid}`} x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%"   stopColor="rgba(255,255,255,0.10)"/>
          <stop offset="100%" stopColor="rgba(0,0,0,0.08)"/>
        </linearGradient>
      </defs>
      <rect x={0} y={0} width={w} height={h} fill={cab.color} stroke={stroke} strokeWidth={sw}/>
      <rect x={0} y={0} width={w} height={h} fill={`url(#cg${tid})`} stroke="none"/>
      {/* Top edge highlight */}
      <line x1={1} y1={1} x2={w-1} y2={1} stroke="rgba(255,255,255,0.3)" strokeWidth="1"/>

      {/* Filler strip */}
      {fillerH>0&&<>
        <rect x={1} y={1} width={w-2} height={fillerH-1} fill="#B0986A" stroke={stroke} strokeWidth="0.5"/>
        <rect x={1} y={1} width={w-2} height={fillerH-1} fill="url(#cg_fill)" stroke="none" opacity="0.4"/>
        <text x={w/2} y={fillerH/2+3} textAnchor="middle" fontSize={Math.max(5,cs*0.45)} fill="#F5F0E8" fontWeight="700" fontFamily="sans-serif" letterSpacing="1">FILLER</text>
      </>}

      {/* Kick plate */}
      {kickH>0&&<>
        <rect x={1} y={h-kickH} width={w-2} height={kickH-1} fill="#786850" stroke={stroke} strokeWidth="0.5"/>
        <rect x={pad} y={h-kickH+2} width={w-pad*2} height={kickH-5} fill="rgba(0,0,0,0.12)" rx="1"/>
        <text x={w/2} y={h-kickH/2+3} textAnchor="middle" fontSize={Math.max(5,cs*0.45)} fill="#C8B898" fontWeight="700" fontFamily="sans-serif" letterSpacing="1">KICK</text>
      </>}
      ...
      {/* Width label */}
      <text x={w/2} y={bodyTop+bodyH-(kickH>0?2:4)} textAnchor="middle"
        fontSize={Math.max(6,cs*0.6)} fill={sel?B.orange:"rgba(80,60,40,0.7)"}
        fontWeight="700" fontFamily="sans-serif">{cab.wMm}mm</text>
    </svg>
  );
}
...
function WallGrid({placed, setPlaced, selType, door, handle, cs, gridCols=60, setGridCols}){
  ...
  function snapPos(col, row, tid) {
    ...
    if (APPLIANCES[tid]) {
      ...
      // Fridges are tall, snap like tall units; others snap to floor
      const isTallApp = ap.hC > 10;
      const sr = isTallApp ? (FLOOR_ROW + 9) - ap.hC : FLOOR_ROW;
      ...
      <div>
        ...
        <div className="overflow-x-auto rounded-xl" style={{border:"1.5px solid #C0B8AC"}}>
          <div className="relative select-none pointer-events-none" style={{width:gridW,height:gridH}}>
            ...
            <div className="absolute" style={{top:0,left:0,right:0,height:floorY,background:"#EDE7DC"}}/>
            <div className="absolute" style={{top:floorY,left:0,right:0,bottom:0,background:"#E8E0D4"}}/>
            {/* Countertop band across floor units */}
            {floorUnits.length>0&&(()=>{
              const sorted=[...floorUnits].sort((a,b)=>a.col-b.col);
              const leftMost=sorted[0].col*previewCs;
              const rightMost=(sorted[sorted.length-1].col+PM.cabinets[sorted[sorted.length-1].tid].wC)*previewCs;
              return <div className="absolute pointer-events-none"
                style={{left:leftMost,top:floorY-previewCs*0.8,height:previewCs*0.8,width:rightMost-leftMost,background:ctM?.color||"#C8C0B0",opacity:0.85,zIndex:6}}/>;
            })()}
            <svg className="absolute inset-0 pointer-events-none" width={gridW} height={gridH}>
        ...
        {floorUnits.map(p=>{
          ...
          return(
            <g key={p.id}>
              ...
              {/* Door swing arcs -- open toward room (downward in plan) */}
              {cab.cat!=="Tall"&&swingR>3&&(
                <>
                  {/* Left door */}
                  <path d={`M${x+3},${frontY} A${swingR},${swingR} 0 0,1 ${x+3+swingR},${frontY+swingR}`}
                    fill="none" stroke="#A09080" strokeWidth="0.6" strokeDasharray="2.5 1.5"/>
                  <line x1={x+3} y1={frontY} x2={x+3+swingR} y2={frontY}
                    stroke="#A09080" strokeWidth="0.4" opacity="0.6"/>
                  {/* Right door */}
                  <path d={`M${x+pw-3},${frontY} A${swingR},${swingR} 0 0,0 ${x+pw-3-swingR},${frontY+swingR}`}
                    fill="none" stroke="#A09080" strokeWidth="0.6" strokeDasharray="2.5 1.5"/>
                  <line x1={x+pw-3} y1={frontY} x2={x+pw-3-swingR} y2={frontY}
                    stroke="#A09080" strokeWidth="0.4" opacity="0.6"/>
                </>
              )}
